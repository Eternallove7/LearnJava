 ...
 *
 * 二、Map结构的理解 :
 *    Map中的key：无序的、不可重复的，使用了Set存储所有的key   ---> key所在的类要重写equals()和 hashCode()  (以HashMap为例)
 ...
 *
 *      在不断的添加过程中，会设计到扩容问题，当超出临界值时(且要存放的位置非空时)默认的扩容方式：扩容为原来容量的2倍，并将原有的数据复制过来。
 *
 *      jdk8相较于jdk7在底层实现方法面的不同：
